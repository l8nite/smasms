...
/**
 * Use analyze data in simpleDB and assign each value with negative, neutral,
 * positive value will add a column named moodVal with values [-1, 0, 1]
 * [negative, neutral, positive] then counts are analyzed and available as
 * public methods Example: AnalysisOfDB myDBAnalyzer = new
 * AnalysisOfDB("Tweets", "Tweet"); int pos = myDBAnalyzer.getPositiveCount();
 * int neu = myDBAnalyzer.getNeutralCount(); int neg =
 * myDBAnalyzer.getNegativeCount(); int tot = myDBAnalyzer.getTotalCount();
 * 
 */
public class AnalysisOfDB {
	...
	/**
	 * Constructor with defined AmazonSimpleDB and supplied domain name and
	 * colName to analyze
	 * 
	 * @param givenDB
	 * @param givenDomain
	...
	/**
	 * Constructor with supplied domain name and colName to analyze, using
	 * default AmazonSimpleDB
	 * 
	 * @param givenDomain
	 * @param givenColName
	 */
	...
	private static void setup() {
		...
		try {
			String selectExpression2 = "select " + colName + " from `"
					+ myDomain + "`";
			...
			for (Item item : sdb.select(selectRequest2).getItems()) {
				...
				for (Attribute attribute2 : item.getAttributes()) {
					...
					// System.out.println("        rg_Name:  " +
					// attribute2.getName());
					// System.out.println("        rg_Value: " +
					// attribute2.getValue());
				...
				moodValInput.add(new ReplaceableItem(item.getName())
						.withAttributes(new ReplaceableAttribute("moodVal", ""
								+ moodValue, true)));
			...
			totalCount = negCount + posCount + newCount;
	...
	/**
	 * input string to analyze, currently only return a random value of [-1, 0,
	 * 1]
	 * 
	 * @param s
	 * @return
	...
	private static void getNeutralCount(AmazonSimpleDB db) {
		String selectExpression2 = "select moodVal from `" + myDomain
				+ "` where moodVal = '0'";
		// System.out.println("Selecting =: " + selectExpression2 + "\n");
		SelectRequest selectRequest2 = new SelectRequest(selectExpression2);
		int count = 0;
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			System.out.println("Number of Neutral values: " + newCount);
			System.out.println("Number of Positive values: " + posCount);
			System.out.println("Total Number of entries considered: "
					+ totalCount);
